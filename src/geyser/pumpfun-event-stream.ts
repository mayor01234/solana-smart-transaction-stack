...
export class PumpfunEventStream extends EventEmitter {
  ...
  private onData(data: any): void {
    ...
    for (const log of logs) {
      ...
      const trade = this.decodeTrade(log.slice(PROGRAM_DATA_PREFIX.length));
      if (!trade) continue;
  ...
  private decodeTrade(b64: string): Omit<PumpfunTradeEvent, 'signature' | 'slot' | 'observedAt'> | undefined {
    let buf: Buffer;
    try {
