...
const staticChecks = [
  ...
  { name: 'Jito bundle implementation exists (jito-ts SDK + JSON-RPC fallback)', weight: 10, pass: hasFile('src/jito/jito-grpc-client.ts') && hasFile('src/jito/jito-jsonrpc-client.ts') && hasFile('src/jito/bundle-builder.ts') },
  ...
  { name: 'AI tip/timing/retry signal providers exist', weight: 6, pass: hasFile('src/agents/tip-intelligence-agent.ts') && hasFile('src/agents/submission-timing-agent.ts') && hasFile('src/agents/retry-reasoning-agent.ts') },
  { name: 'LLM-backed decision agent exists', weight: 4, pass: hasFile('src/agents/transaction-decision-agent.ts') && hasFile('src/agents/llm/anthropic-provider.ts') },
  { name: 'Stream-based commitment tracking exists', weight: 4, pass: hasFile('src/core/commitment-tracker.ts') },
