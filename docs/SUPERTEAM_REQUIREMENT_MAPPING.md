...
| Submit Jito bundles | `src/jito/jito-grpc-client.ts` (official jito-ts SDK), `src/jito/jito-jsonrpc-client.ts` (fallback), `src/core/orchestrator.ts` | Implemented | Uses the recommended jito-ts SDK; bundle ID + real-time bundle results captured in evidence |
...
| Confirm landing using subscriptions | `src/core/lifecycle-stream-tracker.ts`, `src/core/commitment-tracker.ts`, `src/geyser/slot-stream.ts` | Implemented | Confirmed/finalized from the Yellowstone slot-status stream, raced with an RPC signature subscription; never RPC polling |
| Automatic retries with blockhash refresh | `src/core/orchestrator.ts`, `src/agents/transaction-decision-agent.ts` | Implemented | Retry action is produced by the LLM agent's reasoning (not a hardcoded flow) and captured in `agentDecision` |
...
| AI owns one operational decision | `src/agents/transaction-decision-agent.ts`, `src/agents/llm/*` | Implemented | LLM (Claude) owns the action + tip with visible reasoning; deterministic providers feed it and guardrails bound it. Exceeds the requirement (tip + timing + retry) |
